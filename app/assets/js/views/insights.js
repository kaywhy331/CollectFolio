...
export function renderInsights(state) {
  ...
  const subtitles = {
    ...
    forecasts: 'Manual scenario ranges work now; published market forecasts remain a separate evidence tier.',
...
function performanceSection(state) {
  ...
  return `<section class="insights-performance" aria-labelledby="performance-title">
    ...
    <div class="insights-metric-grid">
      ...
      <article><span>Manual values</span><strong>${escapeHTML(formatCurrency(breakdown.manualValue, currency))}</strong><small>${breakdown.manualHoldings} holding${breakdown.manualHoldings === 1 ? '' : 's'} · used only as labeled manual scenario anchors</small></article>
...
function forecastsSection(state) {
  ...
  return `${publicEnabled ? intelligenceStatus(state) : ''}<section class="forecast-workspace">
    ...
    ${localSummary.rows.length ? `<div class="insights-forecast-list">${localSummary.rows.map((row) => localScenarioAssetCard(row, horizon)).join('')}</div>` : emptyState('Add a holding to start', 'A current catalog value or your own estimate creates the first manual scenario anchor.', '<button class="button" type="button" data-go="add">Add collectible</button>')}
    ${publicEnabled ? `${forecastSummaryCard(summary, currency)}
      <div class="section-heading"><div><p class="eyebrow">Published market evidence</p><h2>Approved forecast availability</h2><p class="muted">Published forecasts, when available, stay distinct from your manual scenarios and retain their independent review gates.</p></div></div>
      ${assets.length ? `<div class="insights-forecast-list">${assets.map((asset) => forecastAssetCard(asset, state, latestHistory.get(asset.publication?.variantId), currency)).join('')}</div>` : emptyState('No published forecasts for these items', 'Manual scenarios above remain available without an approved publication.', '<button class="button ghost" type="button" data-go="portfolio">Open portfolio</button>')}` : publicationGateNotice()}
  </section>`;
}
...
function localScenarioSummaryCard(summary, currency) {
  ...
  return `<section class="card portfolio-forecast-summary local-scenario-summary" aria-labelledby="local-scenario-summary-title"><div class="section-heading"><div><p class="eyebrow">Manual scenario outlook</p><h2 id="local-scenario-summary-title">${summary.horizon === 365 ? '1-year' : `${summary.horizon}-day`} portfolio range</h2><p class="muted">Available from values saved on this device. It is not a market appraisal or a published forecast.</p></div><span class="support-badge ${covered ? 'partial' : 'unsupported'}">${covered ? `${summary.coveredHoldings} modeled` : 'Needs a value'}</span></div>
...
function localScenarioAssetCard({ holding, scenario }, horizon) {
  ...
  return `<article class="card forecast-availability-card local-scenario-card ${scenario.status}"><div class="forecast-asset-head">${externalImage(holding.item, 'forecast-product-image')}<div><p class="eyebrow">Owned holding · Manual scenario</p><h3>${escapeHTML(name)}</h3><p class="item-meta">${escapeHTML(identity || 'Saved collectible')}</p></div><span class="support-badge partial">${escapeHTML(scenario.confidence.label)} confidence</span></div>
    <div class="actual-forecast-split"><div class="actual"><span>Saved unit value</span><strong>${escapeHTML(formatCurrency(scenario.observed, scenario.currency))}</strong><small>${escapeHTML(scenario.source === 'manual' ? 'Your estimate' : scenario.sourceLabel || 'Catalog price')} · value date ${escapeHTML(dateLabel(scenario.valueAsOf || scenario.observedAt))}</small></div><div class="forecast"><span>${horizon}-day manual scenario</span><strong>${escapeHTML(formatCurrency(scenario.q25, scenario.currency))}–${escapeHTML(formatCurrency(scenario.q75, scenario.currency))}</strong><small>Middle 50% · midpoint ${escapeHTML(formatCurrency(scenario.q50, scenario.currency))}</small></div></div>${projection}
    ...
    <p class="forecast-warning">Your manual scenario is not a market observation, appraisal, investment recommendation, or guaranteed return.</p><div class="item-actions">${detail}</div></article>`;
}
