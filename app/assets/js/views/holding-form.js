...
function hiddenIdentity(item) {
  return ['name', 'category', 'game', 'setName', 'number', 'variant', 'year']
    .map((name) => `<input type="hidden" name="${name}" value="${escapeAttribute(item[name] || '')}">`)
    .join('');
}
...
function catalogSelection(item, holding, image) {
  ...
  </section><p class="prefill-note"><span aria-hidden="true">✓</span><span><strong>Printing details are already filled in.</strong> Set, number, rarity, artwork, and source stay linked to the catalog result you chose.</span></p>${hiddenIdentity(item)}`;
}

export function renderHoldingForm(holding = null, { image = '', item: proposedItem = null } = {}) {
  ...
  const ownershipOpen = Boolean(holding?.purchaseDate || holding?.fees || holding?.folder);
  ...
  return `<form id="holding-form" class="holding-form">
    ${isCatalogItem ? catalogSelection(item, holding, image) : customIdentity(item)}
    ...
    <details class="form-disclosure" ${detailOpen ? 'open' : ''}><summary><span><strong>Grading, value &amp; notes</strong><small>Only when you need them</small></span><span aria-hidden="true">+</span></summary><div class="form-disclosure-body field-grid">
      ...
      <label class="span-all">Manual market value per item<input name="manualMarketPrice" type="number" min="0" step="0.01" value="${escapeAttribute(holding?.manualMarketPrice ?? '')}" placeholder="Leave blank to use a permitted catalog value"><span class="fine-print">A manual value overrides, but does not erase, the catalog reference.</span></label>
      ...
      <label class="span-all">Your photo<input name="photo" type="file" accept="image/*"><span class="fine-print">The original source photo is never uploaded. This portfolio image stays in IndexedDB.</span></label>
    </div></details>
