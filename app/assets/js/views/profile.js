...
function dateTime(value, fallback = 'Not yet synced') {
...
function accountCopy(state, status) {
  ...
  const copies = {
    ...
    pending: ['Waiting to sync', pending
      ? `${pending} saved change${pending === 1 ? '' : 's'} will be backed up the next time sync succeeds.`
      : 'Your cloud account is connected and ready for its first sync.'],
    syncing: ['Syncing now', 'Local changes remain usable while cloud backup is updated in the background.'],
    synced: ['Synced', `This device last synced ${dateTime(state.settings.lastSyncedAt).toLowerCase()}.`],
    ...
    error: ['Sync needs attention', state.settings.lastSyncError || state.auth.error || 'Your local collection is unchanged. Retry sync when you are ready.']
  };
  return copies[status] || copies.local;
}

function syncHistory(history = []) {
  if (!history.length) return '<p class="muted">No syncs recorded on this device yet.</p>';
  return `<ol class="sync-history">${history.map((entry) => `<li><span class="history-state ${entry.status}">${entry.status === 'success' ? 'Completed' : 'Needs attention'}</span><div><strong>${escapeHTML(entry.summary || (entry.status === 'success' ? 'Sync completed' : "Sync didn't finish"))}</strong><small>${escapeHTML(dateTime(entry.at, 'Date unavailable'))}${entry.reference ? ` · Reference ${escapeHTML(entry.reference)}` : ''}</small></div></li>`).join('')}</ol>`;
}
...
export function renderProfile(state) {
  ...
  return `${pageHeader('Account, preferences, and data', 'Settings')}
    <section class="card account-status-card" data-account-status="${escapeAttribute(shell.syncStatus)}">
      ...
      <div><p class="eyebrow">${session ? `Cloud backup &amp; sync · ${escapeHTML(email)}` : 'Local collection'}</p><h2>${escapeHTML(statusTitle)}</h2><p>${escapeHTML(statusDetail)}</p><div class="status-facts"><span>${state.holdings.length} purchase${state.holdings.length === 1 ? '' : 's'} on device</span>${session ? `<span>${state.auth.pendingChanges || 0} waiting</span><span>Last synced: ${escapeHTML(dateTime(state.settings.lastSyncedAt))}</span>` : ''}</div></div>
    </section>
    <section class="settings-grid account-settings-grid">
      <article class="card settings-panel"><p class="eyebrow">Account &amp; sync</p><h2>${session ? 'Cloud backup is connected' : 'Optional cloud backup'}</h2><p class="muted">${session ? 'Sync merges collection purchases and Watchlist items across signed-in devices. Local changes remain available even if a sync is interrupted.' : configured ? 'Sign in to back up collection purchases and Watchlist items across devices. CollectFolio remains fully usable without an account.' : "Cloud backup isn't available yet. Everything you save remains usable on this device."}</p>
        ...
        <div class="button-row">${session
          ? `<button class="button" type="button" data-action="sync-now" ${state.auth.syncing || !state.auth.online ? 'disabled' : ''}>${state.auth.syncing ? 'Syncing…' : 'Sync now'}</button><button class="button ghost" type="button" data-action="sign-out">Sign out</button>`
          : `<button class="button secondary" type="button" data-action="open-auth" ${configured ? '' : 'disabled'}>Sign in or create account</button>`}</div>
      </article>
      ${session ? `<article class="card settings-panel"><p class="eyebrow">Market data</p><h2>Prioritize my cards</h2><p class="muted">Ask for a sooner price check on your cards. Values update only after new data is verified.</p><button class="button secondary" type="button" data-action="request-price-refresh" ${state.auth.refreshingPrices || !state.auth.online ? 'disabled' : ''}>${state.auth.refreshingPrices ? 'Requesting…' : 'Prioritize my cards'}</button></article>` : ''}
    </section>
    ...
    <section class="card settings-section"><div class="settings-section-heading"><div><p class="eyebrow">Preferences</p><h2>Collection defaults</h2></div><span class="saved-hint" role="status">Changes save on this device</span></div><div class="settings-form-grid">
      ...
      ${state.featureFlags?.publicPriceIntelligence ? `<label>Default forecast horizon<select data-setting="defaultForecastHorizon">${[[7, '7 days'], [30, '30 days'], [90, '90 days'], [180, '180 days'], [365, '365 days']].map(([value, label]) => option(value, state.settings.defaultForecastHorizon, label)).join('')}</select></label>` : ''}
      <label>Preferred market source<select data-setting="preferredMarketSource">${[['all', 'Automatic'], ['tcgcsv', 'Trading card games']].map(([value, label]) => option(value, state.settings.preferredMarketSource, label)).join('')}</select></label>
    </div></section>
    ...
    <section class="card settings-section"><p class="eyebrow">Privacy</p><h2>Private market insights</h2><p class="muted">When you are signed in, exact-card activity can help prioritize future market coverage. Raw activity remains private to your account, and aggregate trends stay hidden until at least 20 distinct collectors contribute. You can opt out at any time.</p><div class="preference-toggles">
      ...
      <label class="setting-toggle"><span><strong>Sync issue notices</strong><small>Show an in-app notice when cloud backup needs attention.</small></span><input type="checkbox" data-setting-toggle="syncIssueNotifications" ${state.settings.syncIssueNotifications ? 'checked' : ''}></label>
    ...
    <section class="card settings-section"><div class="settings-section-heading"><div><p class="eyebrow">Activity</p><h2>Sync history</h2></div><button class="button ghost small" type="button" data-action="reopen-onboarding">Revisit setup guide</button></div>${syncHistory(state.settings.syncHistory)}</section>

    <section class="card danger-zone settings-section"><p class="eyebrow">Data controls</p><h2>Remove saved data</h2><div class="danger-actions"><div><strong>Clear this device</strong><p>Removes local collection purchases, scans, settings, catalog details, cached images and app files, and sync history. Cloud copies are not changed.</p><button class="button danger" type="button" data-action="clear-data">Clear local data</button></div><div><strong>Remove cloud data</strong><p>${cloudDataRemovalEnabled ? 'Removes your cloud collection purchases, Watchlist, scans, private market activity, artwork votes, and history while retaining the sign-in account. Local data stays here and cloud sync disconnects.' : 'Not available yet.'}</p><button class="button danger" type="button" data-action="remove-cloud-data" ${session && state.auth.online && cloudDataRemovalEnabled ? '' : 'disabled'}>Remove cloud data</button></div></div></section>
    <p class="fine-print settings-version">CollectFolio ${escapeHTML(globalThis.window?.COLLECTFOLIO_CONFIG?.APP_VERSION || '0.8.36')}</p>`;
}
