...
function accountCopy(state, status) {
  ...
  const copies = {
    local: ['Saved locally', `${state.holdings.length} holding${state.holdings.length === 1 ? '' : 's'} stays available on this device without an account.`],
    ...
    offline: ['Offline', pending
      ...
      : 'Local holdings remain available. Cloud actions will resume after this device reconnects.'],
    error: ['Synchronization needs attention', state.settings.lastSyncError || 'Your local portfolio is unchanged. Retry synchronization when you are ready.']
  };
...
export function renderProfile(state) {
  ...
  return `${pageHeader('Account, preferences, and data', 'Settings', 'Control what stays on this device, what is backed up, and how CollectFolio works for you.')}
    <section class="card account-status-card" data-account-status="${escapeAttribute(shell.syncStatus)}">
      ...
      <div><p class="eyebrow">${session ? `Cloud backup &amp; sync · ${escapeHTML(email)}` : 'Local portfolio'}</p><h2>${escapeHTML(statusTitle)}</h2><p>${escapeHTML(statusDetail)}</p><div class="status-facts"><span>${state.holdings.length} holding${state.holdings.length === 1 ? '' : 's'} on device</span>${session ? `<span>${state.auth.pendingChanges || 0} waiting</span><span>Last synchronized: ${escapeHTML(dateTime(state.settings.lastSyncedAt))}</span>` : ''}</div></div>
    </section>
    <section class="settings-grid account-settings-grid">
      <article class="card settings-panel"><p class="eyebrow">Account &amp; sync</p><h2>${session ? 'Cloud backup is connected' : 'Optional cloud backup'}</h2><p class="muted">${session ? 'Synchronization merges holdings and Watchlist items across signed-in devices. Local changes remain available even if a sync is interrupted.' : configured ? 'Sign in to back up holdings and Watchlist items across devices. CollectFolio remains fully usable without an account.' : 'Cloud backup is unavailable in this build. Everything you save remains usable on this device.'}</p>
    ...
    <section class="card settings-section"><div class="settings-section-heading"><div><p class="eyebrow">Preferences</p><h2>Collection defaults</h2></div><span class="saved-hint" role="status">Changes save on this device</span></div><div class="settings-form-grid">
      <label>Portfolio currency<select data-setting="currency">${[['USD', 'USD — US Dollar'], ['CAD', 'CAD — Canadian Dollar'], ['EUR', 'EUR — Euro'], ['GBP', 'GBP — Pound sterling']].map(([value, label]) => option(value, state.settings.currency, label)).join('')}</select><span class="fine-print">Totals include matching amounts only; other currencies remain separate without guessed conversion.</span></label>
      ...
      <label>Preferred market source<select data-setting="preferredMarketSource">${[['all', 'Automatic'], ['tcgcsv', 'Full TCGCSV catalog · signed-in test'], ['pokemon', 'Pokémon market'], ['scryfall', 'Magic market'], ['ygoprodeck', 'Yu-Gi-Oh! market']].map(([value, label]) => option(value, state.settings.preferredMarketSource, label)).join('')}</select></label>
    </div></section>
    ...
    <section class="card danger-zone settings-section"><p class="eyebrow">Data controls</p><h2>Remove saved data</h2><div class="danger-actions"><div><strong>Clear this device</strong><p>Removes local holdings, scans, settings, provider metadata, cached images and app files, and sync history. Cloud copies are not changed.</p><button class="button danger" type="button" data-action="clear-data">Clear local data</button></div><div><strong>Remove cloud data</strong><p>${cloudDataRemovalEnabled ? 'Removes your cloud holdings, Watchlist, scans, private market activity, artwork votes, and history while retaining the sign-in account. Local data stays here and cloud sync disconnects.' : 'Unavailable until independently recoverable cloud removal has passed hosted isolation and rollback checks. Local data and normal synchronization remain available.'}</p><button class="button danger" type="button" data-action="remove-cloud-data" ${session && state.auth.online && cloudDataRemovalEnabled ? '' : 'disabled'}>Remove cloud data</button></div></div></section>
    <p class="fine-print settings-version">CollectFolio ${escapeHTML(globalThis.window?.COLLECTFOLIO_CONFIG?.APP_VERSION || '0.8.6')}</p>`;
}
