...
function watchlistSection(state) {
  ...
  return `<section class="metric-grid compact-metrics">
    ...
    ${shown.length ? `<div class="holding-list">${shown.map((entry) => watchlistCard(entry, currency, state.intelligence?.byVariant?.[entry.canonicalVariantId], activeAlerts.filter((alert) => alert.watchKey === entry.watchKey), state.compare || [])).join('')}</div>` : `<section class="empty-state"><span class="empty-symbol">☆</span><h2>${state.watchlistItems.length ? 'No watched cards match' : 'Your watchlist is empty'}</h2><p>${state.watchlistItems.length ? 'Adjust the filters to see more cards.' : 'Watch an exact printing from Search, a holding, or scan review.'}</p><button class="button" type="button" data-go="search">Search cards</button></section>`}`;
}
...
function watchlistCard(entry, currency, publication, alerts = [], compareSelection = []) {
  const ref = entry.catalogRef || {};
