...
function scanDraftControls(state = {}) {
  ...
  if (!drafts.length) {
    return `<button class="draft-resume" type="button" data-action="resume-scan"><span>${icon('resume', { size: 20 })}</span><span><strong>Continue saved review</strong><small>${state.scanDraftCount} local draft${state.scanDraftCount === 1 ? '' : 's'} ready on this device</small></span><span aria-hidden="true">→</span></button>`;
  }
...
export function renderAdd(state) {
  return `${pageHeader('Add items', 'Scan')}
    ...
    <div class="intake-grid unified">
      <button class="intake-card" type="button" data-go="search"><span class="symbol">${icon('search', { size: 20 })}</span><span><h3>Search catalog</h3><p>Find an exact printing across every supported game and catalog.</p></span><span>→</span></button>
      <button class="intake-card" type="button" data-action="import-json"><span class="symbol">${icon('importArrow', { size: 20 })}</span><span><h3>Import collection</h3><p>Merge a CollectFolio backup file.</p></span><span>→</span></button>
      <button class="intake-card" type="button" data-action="custom-holding"><span class="symbol">${icon('plus', { size: 20 })}</span><span><h3>Create custom item</h3><p>Add sports, comics, slabs, sealed products, or anything else.</p></span><span>→</span></button>
    </div>
    ...
    <p class="intake-privacy"><span>${icon('diamond', { size: 16 })}</span><span><strong>Private by default.</strong> Photos stay on this device; only the card crop is sent for identification.</span></p>
