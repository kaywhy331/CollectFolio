...
export function renderOnboarding(state) {
  ...
  const common = `${pageHeader('Three quick steps', 'Set up CollectFolio', 'Start locally in under a minute. An account is always optional.')}${progress(step)}`;
  if (step === 'currency') {
    return `<section class="onboarding-shell">${common}<div class="card onboarding-card"><p class="eyebrow">Step 2 of 3</p><h2>Choose your portfolio currency</h2><p>Totals include only values recorded in this currency. Amounts in other currencies stay labeled and separate; CollectFolio never guesses an exchange rate.</p><form id="onboarding-currency"><label>Display currency<select name="currency">${[
