...
The current application patch is `0.8.31` with service-worker shell `collectfolio-shell-v0.8.31`. Collection-value chart labels remain readable and unclipped across the supported phone, tablet, and desktop viewport matrix. Local database upgrades surface explicit close-other-tabs recovery instead of waiting forever, and current tabs release IndexedDB automatically when a newer app version needs it. Scan input is decoded without a base64 copy of the original, bounded to 8 megapixels and 3,200 px on its longest edge, and converted into at most 900 px crops; the source working image remains only in memory for the active review and is released when that review closes, while IndexedDB v6 removes any legacy persisted source-photo fields. Drafts can be resumed or discarded independently, and the crop editor supports focusable keyboard selection, corner choice, movement, resizing, and deletion. Recognition approval now requires an exact catalog identity: a unique reverse-image bridge can resolve to a TCGCSV product/finish, while visual similarity by itself remains an unapprovable suggestion. Account sync derives identity from the signed token, atomically binds one local collection to one account, rejects foreign remote rows, and uses the hosted migration-0021 `(user_id, id)` holding/session keys. Search sorting uses rights-aware values, trajectory artifacts are bounded and identity-validated, every served quantile band must be finite and noncrossing, and stale forecasts are withheld with an explicit detail-state explanation. Recognition qualification runs on Chromium, Firefox, and WebKit. Anonymous catalog-content access and public/commercial redistribution remain disabled for this personal integration test. Migrations `0015` through `0020` remain intentionally unapplied; migration `0021_account_owned_sync_keys.sql` is applied to the hosted project and documented in [docs/NETLIFY_DEPLOY.md](docs/NETLIFY_DEPLOY.md).
