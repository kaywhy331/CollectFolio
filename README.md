...
The current application patch is `0.8.3` with service-worker shell `collectfolio-shell-v0.8.3`; it adds complete on-demand provider pagination with bounded result rendering and restores the gated daily TCGCSV private-market backbone on top of the `0.8.2` image-identification release. IndexedDB v5 is unchanged and preserves the version-4 migration fixture. Migration `0015_remove_my_cloud_data.sql` is checked in but intentionally not applied; its separate rollout, qualification, and rollback boundary is documented in [docs/REDESIGN_ACCOUNT_SYNC_RELEASE.md](docs/REDESIGN_ACCOUNT_SYNC_RELEASE.md).
