...
The current application patch is `0.8.5` with service-worker shell `collectfolio-shell-v0.8.5`; it adds a restorable local Portfolio Sets view on top of the `0.8.4` game → set → card browser. Set groups deduplicate exact local printings across acquisition lots, retain bounded rendering, and withhold catalog completion percentages until an authoritative total is linked. IndexedDB v5 is unchanged and preserves the version-4 migration fixture. TCGCSV catalog publication remains disabled under its immutable research-only review. Migration `0015_remove_my_cloud_data.sql` is checked in but intentionally not applied; its separate rollout, qualification, and rollback boundary is documented in [docs/REDESIGN_ACCOUNT_SYNC_RELEASE.md](docs/REDESIGN_ACCOUNT_SYNC_RELEASE.md).
