...
- Signed-in full TCGCSV integration-test browsing across all categories, groups, products, finishes, and current price fields, served from private R2 through bounded APIs
...
The current application patch is `0.8.6` with service-worker shell `collectfolio-shell-v0.8.6`; it adds the authenticated full TCGCSV catalog and every current finish-price row to the existing game → set → card browser while retaining unpriced products. The R2 publication is deterministic, hash-addressed, range-readable, and refreshed at most once per upstream source build. IndexedDB v5 is unchanged and preserves the version-4 migration fixture. Anonymous catalog access and public/commercial redistribution remain disabled for this personal integration test. Migration `0015_remove_my_cloud_data.sql` is checked in but intentionally not applied; its separate rollout, qualification, and rollback boundary is documented in [docs/REDESIGN_ACCOUNT_SYNC_RELEASE.md](docs/REDESIGN_ACCOUNT_SYNC_RELEASE.md).
