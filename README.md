...
The current application patch is `0.8.26` with service-worker shell `collectfolio-shell-v0.8.26`; it exposes a deterministic searchable directory of all 90 TCGCSV source categories while keeping imported catalog contents authenticated. The all-games landing performs no set-index requests, a selected game loads only its own newest-first set index, and set covers hydrate in small idle batches. Discover results, set products, and item listings share one responsive, full-title tile template without match or product-family buckets. Catalog views mount at most 48 tiles per page; set-product metadata follows bounded 48-product cursors, and an explicit in-set search completes the remaining metadata pages before filtering while still rendering only the matching page. Forecast and history artifact requests shared by cards in one group are coalesced while in flight, with compact forecast values and no qualifier/disclaimer paragraphs on catalog tiles. Legacy saved TCGCSV identities reconstruct their TCGplayer card art from the persisted product ID when an image URL was not stored. The price timeline has 1M/3M/6M/1Y/All observed-history ranges, an independent latest-forecast toggle, daily forecast hover readings, and calibrated 30/90-day uncertainty checkpoints. The R2 publication is deterministic, hash-addressed, range-readable, and refreshed at most once per upstream source build. IndexedDB v5 is unchanged and preserves the version-4 migration fixture. Anonymous catalog-content access and public/commercial redistribution remain disabled for this personal integration test. Migration `0015_remove_my_cloud_data.sql` is checked in but intentionally not applied; its separate rollout, qualification, and rollback boundary is documented in [docs/REDESIGN_ACCOUNT_SYNC_RELEASE.md](docs/REDESIGN_ACCOUNT_SYNC_RELEASE.md).
