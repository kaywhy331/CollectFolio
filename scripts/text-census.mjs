...
// the visible text each one produces. Used two ways:
//
...
const { renderHome } = await import('../app/assets/js/views/home.js');
const { renderSearch } = await import('../app/assets/js/views/search.js');
const { renderPortfolio } = await import('../app/assets/js/views/portfolio.js');
const { renderPriceIntelligenceDetail } = await import('../app/assets/js/views/price-intelligence-detail.js');
const { renderAdd } = await import('../app/assets/js/views/add.js');
const { getState } = await import('../app/assets/js/core/store.js');
const { catalogReferenceForItem } = await import('../app/assets/js/core/catalog-identity.js');
...
// 1 priced item (gallery view, query set so results render). ---
...
function parseArgs(argv) {
  const flags = { writeBaseline: false, check: false, strict: false, asOf: null };
...
function writeBaseline(counts, asOf) {
  const baseline = {
...
async function main() {
  ...
  if (flags.writeBaseline) {
    const baseline = writeBaseline(counts, flags.asOf);
