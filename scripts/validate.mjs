...
if (packageJSON.version !== '0.8.15' || packageLock.version !== '0.8.15' || packageLock.packages?.['']?.version !== '0.8.15') {
  errors.push('Application and lockfile versions must agree on 0.8.15.');
}
...
if (!runtimeConfig.includes("APP_VERSION: '0.8.15-dev'")) errors.push('Local runtime config must identify the 0.8.15 development build.');
if (!buildScript.includes("process.env.APP_VERSION || '0.8.15'")) errors.push('Production builds must default APP_VERSION to 0.8.15.');
...
if (!serviceWorker.includes("const CACHE = 'collectfolio-shell-v0.8.15'")) errors.push('Service worker cache name must be collectfolio-shell-v0.8.15.');
