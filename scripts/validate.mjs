...
if (packageJSON.version !== '0.8.14' || packageLock.version !== '0.8.14' || packageLock.packages?.['']?.version !== '0.8.14') {
  errors.push('Application and lockfile versions must agree on 0.8.14.');
}
...
if (!runtimeConfig.includes("APP_VERSION: '0.8.14-dev'")) errors.push('Local runtime config must identify the 0.8.14 development build.');
if (!buildScript.includes("process.env.APP_VERSION || '0.8.14'")) errors.push('Production builds must default APP_VERSION to 0.8.14.');
...
if (!serviceWorker.includes("const CACHE = 'collectfolio-shell-v0.8.14'")) errors.push('Service worker cache name must be collectfolio-shell-v0.8.14.');
