...
if (packageJSON.version !== '0.8.28' || packageLock.version !== '0.8.28' || packageLock.packages?.['']?.version !== '0.8.28') {
  errors.push('Application and lockfile versions must agree on 0.8.28.');
}
...
if (!runtimeConfig.includes("APP_VERSION: '0.8.28-dev'")) errors.push('Local runtime config must identify the 0.8.28 development build.');
if (!buildScript.includes("process.env.APP_VERSION || '0.8.28'")) errors.push('Production builds must default APP_VERSION to 0.8.28.');
if (!/^\s+APP_VERSION: 0\.8\.28$/m.test(netlifyDeployWorkflow)) errors.push('Netlify production deploys must bake APP_VERSION 0.8.28.');
...
if (!serviceWorker.includes("const CACHE = 'collectfolio-shell-v0.8.28'")) errors.push('Service worker cache name must be collectfolio-shell-v0.8.28.');
