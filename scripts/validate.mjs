...
if (packageJSON.version !== '0.8.23' || packageLock.version !== '0.8.23' || packageLock.packages?.['']?.version !== '0.8.23') {
  errors.push('Application and lockfile versions must agree on 0.8.23.');
}
...
if (!runtimeConfig.includes("APP_VERSION: '0.8.23-dev'")) errors.push('Local runtime config must identify the 0.8.23 development build.');
if (!buildScript.includes("process.env.APP_VERSION || '0.8.23'")) errors.push('Production builds must default APP_VERSION to 0.8.23.');
...
if (!serviceWorker.includes("const CACHE = 'collectfolio-shell-v0.8.23'")) errors.push('Service worker cache name must be collectfolio-shell-v0.8.23.');
