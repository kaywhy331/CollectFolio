...
if (packageJSON.version !== '0.8.25' || packageLock.version !== '0.8.25' || packageLock.packages?.['']?.version !== '0.8.25') {
  errors.push('Application and lockfile versions must agree on 0.8.25.');
}
...
if (!runtimeConfig.includes("APP_VERSION: '0.8.25-dev'")) errors.push('Local runtime config must identify the 0.8.25 development build.');
if (!buildScript.includes("process.env.APP_VERSION || '0.8.25'")) errors.push('Production builds must default APP_VERSION to 0.8.25.');
...
if (!serviceWorker.includes("const CACHE = 'collectfolio-shell-v0.8.25'")) errors.push('Service worker cache name must be collectfolio-shell-v0.8.25.');
