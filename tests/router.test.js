...
test('browse routes preserve progressive game and set identity without hard-coding providers', () => {
  ...
  assert.deepEqual(root.browse, { game: 'all', setId: '', setSlug: '', sort: 'newest', scope: 'all', productSort: 'price-desc', productKind: 'all' });
  ...
  assert.deepEqual(game.browse, { game: 'pokemon', setId: '', setSlug: '', sort: 'alpha', scope: 'main', productSort: 'price-desc', productKind: 'all' });

  const set = parseAppRoute('/discover/magic/mkm?sort=name');
  assert.equal(set.canonicalPath, '/sets/mkm?game=magic&sort=name');
  assert.deepEqual(set.browse, { game: 'magic', setId: 'mkm', setSlug: '', sort: 'newest', scope: 'all', productSort: 'name', productKind: 'all' });
  ...
  assert.equal(parseAppRoute('/discover/magic/mkm?type=cards').canonicalPath, '/sets/mkm?game=magic&type=cards');
  assert.equal(parseAppRoute('/discover/magic/mkm?type=bogus').browse.productKind, 'all');
  assert.equal(parseAppRoute('/discover/pokemon?type=sealed').browse.productKind, 'all');
...
test('legacy view mappings create restorable route state', () => {
  ...
  const sealedBrowse = routeStatePatch(parseAppRoute('/discover/pokemon/swsh12?type=sealed'), {
    ...state,
    discover: { mode: 'browse', game: 'magic', setId: 'mkm', products: [{ id: 'stale' }], productKind: 'cards' }
  });
  assert.equal(sealedBrowse.discover.productKind, 'sealed');
