...
test('service worker refreshes runtime config, bounds caches, and reloads offline', async ({ page, context }) => {
  ...
  await page.reload();
  await expect(page.getByRole('heading', { name: /Set up CollectFolio|Home/ })).toBeVisible();
  ...
  expect(installed).toContain('collectfolio-shell-v0.8.35');

  const refreshedConfig = await page.evaluate(async () => {
    const cache = await caches.open('collectfolio-shell-v0.8.35');
  ...
  await expect.poll(() => page.evaluate(() => caches.keys())).toContain('collectfolio-shell-v0.8.35');
